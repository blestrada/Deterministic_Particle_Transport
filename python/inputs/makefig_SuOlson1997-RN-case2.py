...
for i in range(3):
    ...
    plt.plot(xdata, radnrgdens, color=colors[i], marker='x', label=f"Radiation at {times[i]} (IMC)", linewidth=0.6, markersize='5', alpha=0.5)
...
for i in range(3):
    ...
    plt.plot(xdata, matnrgdens, color=colors[i], marker='x', label=f"Material at {times[i]} (IMC)", linewidth=0.6, markersize='5', alpha=0.5)
...
plt.plot(x_bench, mat_benchtwo[4], 'o', label=r'Material at $\tau$ = 10 (Benchmark)', linewidth=0.6, color='forestgreen', fillstyle='none')  
