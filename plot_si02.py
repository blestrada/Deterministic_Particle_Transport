...
epsilon = 5e-5
r_points = np.array([epsilon+5e-4, 0.08 - epsilon])
...
available_times = df_dpt['time'].unique()
target_time = available_times[-1] # -1 for final, -501 for 0.35 sh

# 1. Filter the dataframe for that specific time
df_step_dpt = df_dpt[df_dpt['time'] == target_time]
df_step_imc = df_imc[df_imc['time'] == target_time]
...
imc_temp_grid = df_step_imc.pivot(index='x_idx', columns='y_idx', values='temp').values
imc_radtemp_grid = df_step_imc.pivot(index='x_idx', columns='y_idx', values='radtemp').values
...
for i, (zi, ri) in enumerate(axis_pairs):
    ...
    d_label = "DPT" if i == 0 else None
    i_label = "IMC" if i == 0 else None

    # Plot using the conditional labels
    plt.plot(dpt_subset['time'], dpt_subset['temp'], color='blue', linestyle='dashed', label=d_label)
    plt.plot(imc_subset['time'], imc_subset['temp'], color='red', label=i_label)
